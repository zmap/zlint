...
func init() {
	...
	flag.BoolVar(&listProfiles, "list-profiles", false, "Prints a line delimited list of JSON. Each line prints name, description, source, and a list of all lints that comprise the profile")
	flag.BoolVar(&summary, "summary", false, "Prints a succinct, tabular, human-readable, summary report in place of the default JSON report. Only counts of info/warn/error/fatal occurrences are reported")
	...
	flag.StringVar(&includeSources, "includeSources", "", "Comma-separated list of lint sources to include. The names provided must be precise. If you wish to use a pattern instead, please see -nameFilter")
	flag.StringVar(&excludeSources, "excludeSources", "", "Comma-separated list of lint sources to exclude. The names provided must be precise. If you wish to use a pattern instead, please see -nameFilter")
