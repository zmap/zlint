...
func (l *sc081ThirdDate46ServerCertValidityTooLong) Execute(c *x509.Certificate) *lint.LintResult {
	maxValidity := 46 * util.DAY_LENGTH

	certValidity := c.NotAfter.Add(1 * time.Second).Sub(c.NotBefore)

	if certValidity > maxValidity {
		return &lint.LintResult{Status: lint.Warn, Details: fmt.Sprintf("Certificate is issued on or after March 15, 2029 and has a vailidity of %d days", certValidity)}
	}
