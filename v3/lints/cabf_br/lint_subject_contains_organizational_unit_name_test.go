...
func TestSubjectContainsOrganizationalUnitName(t *testing.T) {
	testCases := []struct {
		...
		{
			Name:            "Subject contains organizational unit name after application date",
		...
		{
			Name:            "Subject contains deprecated organizational unit name",
			InputFilename:   "subjectDnWithDeprecatedOuEntry.pem",
			ExpectedResult:  lint.Notice,
			ExpectedDetails: "subject:organizationalUnitName is deprecated",
		},
	}
