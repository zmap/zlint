...
func TestQcNaturalPersonKUCorrectSetting(t *testing.T) {
	testCases := []struct {
		Name            string
		...
		{
			Name:           "Pass - certificate is issued to a natural person and has keu usage extension with allowed value",
		...
		{
			Name:            "Error - certificate is issued to a natural person and has keu usage extension with dataEncipherment",
			InputFilename:   "etsi/qcNaturallWithDisallowedKU.pem",
			ExpectedResult:  lint.Error,
			ExpectedDetails: "KeyUsage [DataEncipherment] (00001000) is not allowed for ETSI natural person certificates",
		},
	}
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			result := test.TestLint("e_etsi_natural_person_key_usage_correct_values", tc.InputFilename)
