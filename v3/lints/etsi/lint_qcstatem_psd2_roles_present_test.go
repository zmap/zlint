...
func TestEtsiQcStatemPsd2RolesPresent(t *testing.T) {
	cases := map[string]struct {
		status  lint.LintStatus
		details string // empty means "don't check Details"
	}{
		"QcStmtEtsiPsd2ValidCert01.pem":         {status: lint.Pass},
		"QcStmtEtsiPsd2RolesEmptyCert01.pem":    {status: lint.Error, details: "RolesOfPSP must contain at least one role"},
	...
	for inputPath, tc := range cases {
		out := test.TestLint("e_qcstatem_psd2_roles_present", inputPath)
