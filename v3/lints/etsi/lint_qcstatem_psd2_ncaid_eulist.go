...
// Warn, not Error. IS and IE both use NCA code "CBI" (Central Bank of
// Iceland vs. Central Bank of Ireland) — not a typo; lookups always match
// on CountryCode first, so this causes no ambiguity.
var psd2EuNcaIds = []struct {
	CountryCode string
	NcaCode     string
}{
...
func (l *qcStatemPsd2NcaIdEulist) Execute(c *x509.Certificate) *lint.LintResult {
	...
	for _, entry := range psd2EuNcaIds {
		if entry.CountryCode != countryPrefix {
			continue
		}
		expected := entry.CountryCode + "-" + entry.NcaCode
		if ncaId != expected {
			return &lint.LintResult{Status: lint.Warn, Details: fmt.Sprintf(
				...
				ncaId, expected, entry.CountryCode)}
		}
		return &lint.LintResult{Status: lint.Pass}
	}
	return &lint.LintResult{Status: lint.Pass}
