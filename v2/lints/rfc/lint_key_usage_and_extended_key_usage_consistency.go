...
type keyUsageAndExtendedKeyUsageConsistency struct{}

func (l *keyUsageAndExtendedKeyUsageConsistency) Initialize() error {
	return nil
}

// CheckApplies returns true when the certificate contains both a key usage
// extension and an extended key usage extension.
func (l *keyUsageAndExtendedKeyUsageConsistency) CheckApplies(c *x509.Certificate) bool {
...
// being linted are not consistent with both extensions.
func (l *keyUsageAndExtendedKeyUsageConsistency) Execute(c *x509.Certificate) *lint.LintResult {
...
func init() {
	lint.RegisterLint(&lint.Lint{
		Name:          "e_key_usage_and_extended_key_usage_consistency",
		...
		Lint:          &keyUsageAndExtendedKeyUsageConsistency{},
	})
}
