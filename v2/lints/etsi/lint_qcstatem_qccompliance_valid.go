...
func (l *qcStatemQcComplianceValid) CheckApplies(c *x509.Certificate) bool {
	...
	if util.ParseQcStatem(util.GetExtFromCert(c, util.QcStateOid).Value, util.IdEtsiQcsQcCompliance).IsPresent() {
		return true
	}
	return false
}
...
func (l *qcStatemQcComplianceValid) Execute(c *x509.Certificate) *lint.LintResult {
	...
	s := util.ParseQcStatem(ext.Value, util.IdEtsiQcsQcCompliance)
