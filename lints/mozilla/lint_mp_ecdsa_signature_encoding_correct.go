...
This lint covers the next part. The previous part is covered by e_mp_ecdsa_pub_key_encoding_correct.
...
func (l *ecdsaSignatureAidEncoding) CheckApplies(c *x509.Certificate) bool {
	...
	return c.SignatureAlgorithm == x509.ECDSAWithSHA1 ||
		...
		c.SignatureAlgorithmOID.Equal(asn1.ObjectIdentifier{1, 2, 840, 10045, 4, 3, 1})
}
...
func (l *ecdsaSignatureAidEncoding) Execute(c *x509.Certificate) *lint.LintResult {
	...
	// Thus we need the issuer public key which it is not that easy available .
