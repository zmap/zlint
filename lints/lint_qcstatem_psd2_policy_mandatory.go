...
func (l *qcStatemPsd2PolicyMandatory) Execute(c *x509.Certificate) *LintResult {
	...
	if psd2Present && !QcTypePresent {
		return &LintResult{Status: Error, Details: "EU Qualified Certificate has PSD2 QcStatement but not the mandatory QcType Statement (ETSI EN 319 412-5: \"When the certificate is issued in accordance with Annex III or Annex IV of Regulation (EU) No 910/2014 [i.8], this statement shall be present\")."}
	}
	if util.HasCertAnyEtsiQcStatement(c) && len(c.PolicyIdentifiers) == 0 {
		return &LintResult{Status: Error, Details: "EU Qualified Certificate must at least contain one policy identifier (ETSI EN 319 411-2: GEN-6.6.1-05)"}
	}
