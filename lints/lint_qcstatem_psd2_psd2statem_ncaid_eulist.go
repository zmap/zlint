...
func (l *qcStatemPsd2Psd2StatemNcaidEulist) Execute(c *x509.Certificate) *LintResult {
	...
	if s.GetErrorInfo() != "" {
		return &LintResult{Status: Warn, Details: "parsing error for PSD2 QcStatement, cannot properly apply this lint: " + s.GetErrorInfo()}
	}
	...
	if psd2Statem.GetNcaCountry() == "" || psd2Statem.GetNcaId() == "" {
		return &LintResult{Status: Warn, Details: "NCAId field (country-NcaId) in PSD2 QcStatement has invalid format, cannot properly apply this lint"}
	}
	for _, countryAndNCAId := range countryAndNCAIdMap {
		if psd2Statem.GetNcaCountry() == countryAndNCAId.country {
			if psd2Statem.GetNcaId() != countryAndNCAId.ncaid {
				...
				return &LintResult{Status: Warn, Details: "NCAId in PSD2 QcStatement for given country is not conforming to (informative) EU List"}
			}
		}
	}
