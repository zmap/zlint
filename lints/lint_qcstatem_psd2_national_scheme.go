...
func (l *qcStatemPsd2NationalScheme) Execute(c *x509.Certificate) *LintResult {
	...
	orgId := util.GetSubjectOrgId(c.RawSubject)
	if !orgId.IsPresent {
		return &LintResult{Status: Error, Details: "missing mandatory subject:OrganizationIdentifier"}
	}
	if orgId.ErrorString != "" {
		return &LintResult{Status: Error, Details: orgId.ErrorString}
	}
	if !util.CheckNationalScheme(orgId.Value) {
		return &LintResult{Status: Error, Details: "invalid format of subject:organizationIdentifier for national scheme"}
	}
	errStr, isPresent := util.IsQcStatemPresent(c, &util.IdQcsPkixQCSyntaxV2)
	if errStr != "" {
		return &LintResult{Status: Error, Details: "error parsing IdQcsPkixQCSyntaxV2 Qc Statement"}
	}

	if !isPresent {
		return &LintResult{Status: Error, Details: "national scheme requires URI in IdQcsPkixQCSyntaxV2 Qc Statement, but this Qc Statement is not present"}
	}
	...
	for _, x := range qcs2.Decoded.NameRegAuthorities {
		...
		if err != nil {
			return &LintResult{Status: Error, Details: err.Error()}
		}
		if len(rest) != 0 {
			return &LintResult{Status: Error, Details: "Trailing bytes after URI"}
		}
		return &LintResult{Status: Pass}

	}

	return &LintResult{Status: Error, Details: "did not find URI element within IdQcsPkixQCSyntaxV2 Qc Statement, which is mandatory for the national scheme format of the subject:organizationIdentifier"}
}
