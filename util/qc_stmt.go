...
func etsiOidToDescString(oi asn1.ObjectIdentifier) string {
	switch {
	case oi.Equal(IdEtsiQcsQcCompliance):
		{
			return "IdEtsiQcsQcCompliance"
		}
	case oi.Equal(IdEtsiQcsQcLimitValue):
		{
			return "IdEtsiQcsQcLimitValue"
		}
	case oi.Equal(IdEtsiQcsQcRetentionPeriod):
		{
			return "IdEtsiQcsQcRetentionPeriod"
		}
	case oi.Equal(IdEtsiQcsQcSSCD):
		{
			return "IdEtsiQcsQcSSCSD"
		}
	case oi.Equal(IdEtsiQcsQcEuPDS):
		{
			return "IdEtsiQcsQcEuPDS"
		}
	case oi.Equal(IdEtsiQcsQcType):
		{
...
type qcStatementWoInfoField struct {
	Oi asn1.ObjectIdentifier
}
...
func ParseQcStatem(extVal []byte, sought asn1.ObjectIdentifier) EtsiQcStmtIf {
	...
	for _, raw := range sl {
		...
		if err != nil {
			var statemWoInfo qcStatementWoInfoField

			rest, err = asn1.Unmarshal(raw.Raw, &statemWoInfo)
			if err != nil || len(rest) != 0 {
				return etsiBase{errorInfo: parseErrorString, isPresent: false}
			}
			copy(statem.Oi, statemWoInfo.Oi)
		...
		if !statem.Oi.Equal(sought) {
			continue
		}
		if statem.Oi.Equal(IdEtsiQcsQcCompliance) {
			etsiObj := Etsi421QualEuCert{etsiBase: etsiBase{isPresent: true}}
			statemWoInfo := qcStatementWoInfoField{Oi: statem.Oi}
			AppendToStringSemicolonDelim(&etsiObj.errorInfo, checkAsn1Reencoding(reflect.ValueOf(statemWoInfo).Interface(), raw.Raw,
				"invalid format of ETSI Complicance statement"))
			return etsiObj
		} else if statem.Oi.Equal(IdEtsiQcsQcLimitValue) {
		...
		} else if statem.Oi.Equal(IdEtsiQcsQcRetentionPeriod) {
			...
			return etsiObj
		} else if statem.Oi.Equal(IdEtsiQcsQcType) {
